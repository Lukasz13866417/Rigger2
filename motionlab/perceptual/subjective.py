...
FREE_CAMERA_SUBJECTIVE_PROTOCOL_VERSION = "motionlab.subjective_protocol.v4"
...
# Camera affordances alter viewing conditions, so they have their own immutable protocol.
# Do not change the v1 settings: existing observations and frozen artifacts address them.
FREE_CAMERA_VIEWING_SETTINGS: dict[str, Any] = deepcopy(MANNEQUIN_VIEWING_SETTINGS)
FREE_CAMERA_VIEWING_SETTINGS.update(
    {
        "protocol": "motionlab.fixed_mannequin_canvas.v2",
        "default_camera": {
            **MANNEQUIN_VIEWING_SETTINGS["default_camera"],
            "pan_x_m": 0.0,
            "pan_y_m": 0.0,
        },
        "zoom_limits": {"minimum": 0.1, "maximum": 5.0, "button_factor": 1.25},
        "wheel_zoom": {
            "enabled": True,
            "exponent_per_pixel": 0.0015,
            "line_height_px": 16.0,
            "gesture_gap_ms": 250.0,
            "maximum_delta_px": 1200.0,
        },
        "orbit": {
            "enabled": True,
            "maximum_pitch_degrees": 89.0,
            "yaw_degrees_per_pixel": 0.35,
            "pitch_degrees_per_pixel": 0.35,
        },
        "pan": {
            "enabled": True,
            "maximum_offset_m": 20.0,
            "controls": ["shift_left_drag", "middle_drag", "right_drag"],
            "coordinates": "screen_right_and_up_meters",
        },
    }
)
...
VIEWING_PROTOCOLS: dict[str, dict[str, Any]] = {
    ...
    FREE_CAMERA_VIEWING_SETTINGS["protocol"]: FREE_CAMERA_VIEWING_SETTINGS,
}
...
def _default_camera_state(viewing_settings: dict[str, Any]) -> dict[str, Any]:
    ...
    if viewing_settings.get("pan", {}).get("enabled"):
        state.update({"pan_x_m": float(raw["pan_x_m"]), "pan_y_m": float(raw["pan_y_m"])})
    return state
...
def _validate_camera_state(
    ...
    pan_enabled = bool(viewing_settings.get("pan", {}).get("enabled"))
    if pan_enabled:
        required.update({"pan_x_m", "pan_y_m"})
    ...
    if pan_enabled:
        maximum_offset = float(viewing_settings["pan"]["maximum_offset_m"])
        for key in ("pan_x_m", "pan_y_m"):
            if not _finite_number(value[key]) or abs(float(value[key])) > maximum_offset:
                raise ValueError("camera pan is outside the permitted range")
...
def _camera_is_default(
    ...
    return (
        ...
        and all(
            math.isclose(value.get(key, float("inf")), expected[key], abs_tol=1.0e-9)
            for key in ("pan_x_m", "pan_y_m")
            if key in expected
        )
    )
...
def _validate_inspection_telemetry(
    ...
    *,
    ...
    free_camera = viewing_settings["protocol"] == FREE_CAMERA_VIEWING_SETTINGS["protocol"]
    if free_camera:
        permitted_camera_events.add("pan")
    ...
    for event in camera_events:
        ...
        if free_camera:
            _validate_camera_state(event["from"], viewing_settings)
            _validate_camera_state(event["to"], viewing_settings)
    counts: dict[str, int] = {}
...
def _two_cycle_payload(
    ...
    *,
    ...
    if settings["protocol"] in {
        MANNEQUIN_VIEWING_SETTINGS["protocol"],
        FREE_CAMERA_VIEWING_SETTINGS["protocol"],
    }:
        missing_roles = [
...
function supportsInspection(){return trial&&['motionlab.fixed_mannequin_canvas.v1','motionlab.fixed_mannequin_canvas.v2'].includes(trial.viewing_settings.protocol)}
function supportsFreeCamera(){return trial&&trial.viewing_settings.protocol==='motionlab.fixed_mannequin_canvas.v2'}
function cameraEnabled(){return supportsInspection()&&!ratingLocked&&allComplete()}
function cameraSnapshot(){let state={preset:camera.preset,yaw_degrees:camera.yaw_degrees,pitch_degrees:camera.pitch_degrees,zoom:camera.zoom,skeleton_overlay:camera.skeleton_overlay};if(supportsFreeCamera()){state.pan_x_m=camera.pan_x_m;state.pan_y_m=camera.pan_y_m}return state}
function defaultCamera(){let raw=trial.viewing_settings.default_camera||{preset:'legacy',yaw_degrees:25,pitch_degrees:0,zoom:1,skeleton_overlay:false},state={preset:String(raw.preset),yaw_degrees:Number(raw.yaw_degrees),pitch_degrees:Number(raw.pitch_degrees),zoom:Number(raw.zoom),skeleton_overlay:Boolean(raw.skeleton_overlay)};if(supportsFreeCamera()){state.pan_x_m=Number(raw.pan_x_m);state.pan_y_m=Number(raw.pan_y_m)}return state}
function resetTelemetry(){orbitStart=null;wheelGesture=null;playbackRate=1;camera=defaultCamera();inspectionStartMs=null;lockedInspectionTimeMs=null;speedEvents=[];cameraEvents=[];zoomChanges=0;viewChanges=0;overlayChanges=0;timeBySpeed={};for(const rate of(trial.viewing_settings.playback_rates||[1]))timeBySpeed[String(Number(rate))]=0;$('skeletonOverlay').checked=false;document.querySelector('#inspection .muted').textContent=supportsFreeCamera()?'Drag to orbit · Scroll to zoom · Shift-drag or middle/right-drag to pan':'Drag the viewport to orbit.';updateControlHighlights()}
...
function recordCamera(kind,from,to){wheelGesture=null;if(JSON.stringify(from)===JSON.stringify(to))return null;let event={elapsed_ms:elapsedInspection(),kind:kind,from:from,to:to};cameraEvents.push(event);if(kind==='zoom')zoomChanges++;if(kind==='view')viewChanges++;if(kind==='overlay')overlayChanges++;return event}
...
function changeSpeed(rate){if(ratingLocked||!allComplete()||!(trial.viewing_settings.playback_rates||[]).includes(rate)||rate===playbackRate)return;let previous=playbackRate;playbackRate=rate;speedEvents.push({elapsed_ms:elapsedInspection(),from_rate:previous,to_rate:rate});updateControlHighlights()}
function setView(name){if(!cameraEnabled())return;let preset=trial.viewing_settings.camera_presets[name];if(!preset)return;finishOrbit();let previous=cameraSnapshot();camera.preset=name;camera.yaw_degrees=Number(preset.yaw_degrees);camera.pitch_degrees=Number(preset.pitch_degrees);recordCamera('view',previous,cameraSnapshot());updateControlHighlights();draw()}
function boundedZoom(value){let limits=trial.viewing_settings.zoom_limits;return Math.max(Number(limits.minimum),Math.min(Number(limits.maximum),value))}
function changeZoom(direction){if(!cameraEnabled())return;finishOrbit();let limits=trial.viewing_settings.zoom_limits,previous=cameraSnapshot(),next=boundedZoom(supportsFreeCamera()?camera.zoom*Math.pow(Number(limits.button_factor),direction):camera.zoom+direction*Number(limits.step));if(next===camera.zoom)return;camera.zoom=Number(next.toFixed(6));recordCamera('zoom',previous,cameraSnapshot());draw()}
function resetCamera(){if(!cameraEnabled())return;finishOrbit();let previous=cameraSnapshot();camera=defaultCamera();$('skeletonOverlay').checked=camera.skeleton_overlay;recordCamera('reset',previous,cameraSnapshot());updateControlHighlights();draw()}
function changeOverlay(){if(!cameraEnabled()){$('skeletonOverlay').checked=camera.skeleton_overlay;return}finishOrbit();let previous=cameraSnapshot();camera.skeleton_overlay=$('skeletonOverlay').checked;recordCamera('overlay',previous,cameraSnapshot());draw()}
function lockRating(){if(selected===null||!allComplete()||ratingLocked)return;finishOrbit();wheelGesture=null;ratingLocked=true;playing=false;lockedInspectionTimeMs=performance.now();setInspectionEnabled(false);$('play').disabled=true;document.querySelectorAll('[data-value]').forEach(x=>x.disabled=true);$('continueRating').disabled=true;$('continueRating').classList.add('hidden');if(trial.ask_post_rating_difficulty)$('difficulty').classList.remove('hidden');$('submit').classList.remove('hidden');$('submit').disabled=false}
...
const canvas=$('view');
canvas.addEventListener('contextmenu',event=>{if(supportsFreeCamera())event.preventDefault()});
canvas.addEventListener('pointerdown',event=>{if(!cameraEnabled()||orbitStart)return;let pan=supportsFreeCamera()&&trial.viewing_settings.pan.enabled&&(event.button===1||event.button===2||(event.button===0&&event.shiftKey));if(!pan&&(event.button!==0||!trial.viewing_settings.orbit.enabled))return;event.preventDefault();wheelGesture=null;orbitStart={x:event.clientX,y:event.clientY,state:cameraSnapshot(),pointerId:event.pointerId,kind:pan?'pan':'orbit'};canvas.setPointerCapture(event.pointerId)});
canvas.addEventListener('pointermove',event=>{if(!orbitStart||event.pointerId!==orbitStart.pointerId||!cameraEnabled())return;let dx=event.clientX-orbitStart.x,dy=event.clientY-orbitStart.y,settings=trial.viewing_settings;if(orbitStart.kind==='pan'){let rect=canvas.getBoundingClientRect(),scale=Number(settings.pixels_per_meter)*camera.zoom,maximum=Number(settings.pan.maximum_offset_m);camera.pan_x_m=Math.max(-maximum,Math.min(maximum,orbitStart.state.pan_x_m+dx*canvas.width/rect.width/scale));camera.pan_y_m=Math.max(-maximum,Math.min(maximum,orbitStart.state.pan_y_m-dy*canvas.height/rect.height/scale))}else{let maximum=Number(settings.orbit.maximum_pitch_degrees);if(dx===0&&dy===0)return;camera.preset='orbit';camera.yaw_degrees=orbitStart.state.yaw_degrees+dx*Number(settings.orbit.yaw_degrees_per_pixel||.35);camera.pitch_degrees=Math.max(-maximum,Math.min(maximum,orbitStart.state.pitch_degrees-dy*Number(settings.orbit.pitch_degrees_per_pixel||.25)))}updateControlHighlights();draw()});
function finishOrbit(event){if(!orbitStart||(event&&event.pointerId!==orbitStart.pointerId))return;let gesture=orbitStart;orbitStart=null;recordCamera(gesture.kind,gesture.state,cameraSnapshot());if(canvas.hasPointerCapture(gesture.pointerId))canvas.releasePointerCapture(gesture.pointerId)}
canvas.addEventListener('pointerup',finishOrbit);canvas.addEventListener('pointercancel',finishOrbit);canvas.addEventListener('lostpointercapture',finishOrbit);window.addEventListener('blur',()=>{finishOrbit();wheelGesture=null});
canvas.addEventListener('wheel',event=>{if(!supportsFreeCamera())return;event.preventDefault();if(!cameraEnabled()||!trial.viewing_settings.wheel_zoom.enabled)return;finishOrbit();let settings=trial.viewing_settings.wheel_zoom,units=event.deltaMode===1?Number(settings.line_height_px):event.deltaMode===2?canvas.getBoundingClientRect().height:1,delta=Math.max(-Number(settings.maximum_delta_px),Math.min(Number(settings.maximum_delta_px),event.deltaY*units));if(!Number.isFinite(delta)||delta===0)return;let previous=cameraSnapshot(),next=boundedZoom(camera.zoom*Math.exp(-delta*Number(settings.exponent_per_pixel)));if(next===camera.zoom)return;camera.zoom=next;let now=performance.now();if(wheelGesture&&now-wheelGesture.lastMs<=Number(settings.gesture_gap_ms)){wheelGesture.event.to=cameraSnapshot();wheelGesture.event.elapsed_ms=elapsedInspection();wheelGesture.lastMs=now}else{let recorded=recordCamera('zoom',previous,cameraSnapshot());wheelGesture={event:recorded,lastMs:now}}draw()},{passive:false});
...
def serve_subjective_evaluator(
    ...
    *,
    ...
    if manifest.get("protocol_version") in {
        ...
        # Collected observations retain their exact browser/server implementation.
        from motionlab.perceptual.subjective_v3 import (
            serve_subjective_evaluator as serve_frozen_evaluator,
        )

        return serve_frozen_evaluator(
    ...
    if (
        manifest.get("protocol_version") != FREE_CAMERA_SUBJECTIVE_PROTOCOL_VERSION
        or viewing_settings != FREE_CAMERA_VIEWING_SETTINGS
    ):
        raise ValueError("free-camera collection requires its versioned viewing protocol")
    uses_mannequin = True
    ...
    inherited_history: list[dict[str, Any]] = []
    if manifest.get("camera_continuation") is not None:
        from motionlab.perceptual.camera_continuation import load_camera_continuation_history
        from motionlab.perceptual.observation_auth import resolve_frozen_evidence_paths

        resolve_frozen_evidence_paths(
            pilot_path,
            observations_path,
            pilot_path.with_name("protocol_freeze.json"),
            served_playlist_path=served_path,
        )
        inherited_history = load_camera_continuation_history(manifest)

    def observations() -> list[dict[str, Any]]:
        # Historical responses inform scheduling only. They are never relabelled or
        # persisted as observations of this camera protocol.
        return inherited_history + _read_jsonl(observations_path)
                ...
                if request_path == "/api/session":
                    ...
                    sessions[token] = {
                        ...
                        "completed_tutorial_ids": (
                            {
                                str(item["tutorial_id"])
                                for item in manifest.get("calibration_tutorial", [])
                            }
                            if any(
                                row.get("rater_id") == rater_id and row.get("session_index") == 0
                                for row in observations()
                            )
                            else set()
                        ),
