"""Bounded pre-answer replays and mild speed adjustment, with server-side enforcement."""
...
from motionlab.noticeability.repeated_protocol import (
    MAX_PRIMARY_VIEWINGS,
    PRIMARY_RATES,
    ...
    inside,
    now,
    primary_viewing_policy,
    read_json,
    read_rows,
    validate_optional,
    validate_pilot,
)
from motionlab.noticeability.repeated_ui import HTML
...
def make_server(manifest_path: Path, *, port: int = 8765) -> ThreadingHTTPServer:
    ...
    lock = threading.Lock()
    inherited = (
        read_json(inside(root, manifest["continuation_history"]))
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def dispatch(
            ...
            if route == "next":
                ...
                events = read_rows(root / "observations.jsonl")
                    ...
                    r["trial_id"]
                    for r in events
                    if r["event_type"] == "repeated_view_notice"
                    and r["rater_id"] == state["rater_id"]
                }
                inherited_done = set(
                    inherited.get("completed_trial_ids_by_rater", {}).get(state["rater_id"], [])
                )
                done |= inherited_done
                        ...
                        history = [
                            {
                                ...
                                "spontaneous_notice": r["repeated_view_notice"],
                            }
                            for r in events
                            if r["event_type"] == "repeated_view_notice"
                            and r["rater_id"] == state["rater_id"]
                ...
                state["active"] = {"trial": trial, "item": item, "stage": "ready", "viewings": []}
            ...
            if route == "start":
                if active["stage"] not in {"ready", "question"}:
                    raise ValueError("finish the current viewing before starting another")
                if len(active["viewings"]) >= MAX_PRIMARY_VIEWINGS:
                    raise ValueError("at most three pre-answer viewings are allowed")
                rate = value.get("playback_rate", 1.0)
                if type(rate) not in (float, int) or rate not in PRIMARY_RATES:
                    raise ValueError("pre-answer playback speed must be 0.85, 0.9, 0.95 or 1.0")
                active.update(
                    stage="initial",
                    started=time.monotonic(),
                    started_utc=now(),
                    viewing_index=len(active["viewings"]) + 1,
                ...
                if (
                    ...
                    or time.monotonic() - active["started"]
                    < (item["duration_s"] - 1 / item["fps"]) / active["playback_rate"]
                ):
                    raise ValueError("complete the full presentation at the selected speed")
                active.update(stage="question", completed=time.monotonic(), completed_utc=now())
                active["viewings"].append(
                    {
                        "viewing_index": active["viewing_index"],
                        "playback_rate": active["playback_rate"],
                        "content_duration_s": float(item["duration_s"]),
                        "wall_duration_s": active["completed"] - active["started"],
                ...
                row = PrimaryObservation(
                    ...
                    **{
                    ...
                    repeated_view_notice=value["repeated_view_notice"],
                    primary_response_time_ms=(time.monotonic() - active["completed"]) * 1000,
                    pre_answer_viewings=active["viewings"],
        ...
        def evidence(self, state: dict[str, Any], event: str) -> None:
            ...
            if event in {"started", "completed"}:
                row.update(
                    viewing_index=active["viewing_index"], playback_rate=active["playback_rate"]
                )
                row["timestamp_utc"] = (
                    active["started_utc"] if event == "started" else active["completed_utc"]
                )
