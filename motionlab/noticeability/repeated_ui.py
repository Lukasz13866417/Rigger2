"""Up to three mildly speed-adjusted viewings before an immutable noticeability answer."""

from motionlab.noticeability.repeated_protocol import BODY_PARTS, PRIMARY_QUESTION, REASON_TAGS
...
<p>Watch up to three times before answering. You can use 0.85×–1× speed to catch anything you missed. Detailed inspection comes afterward.</p>
...
<div id="preControls" hidden><label>Viewing speed <select id="preSpeed" disabled><option value="0.85">0.85×</option><option value="0.9">0.90×</option><option value="0.95">0.95×</option><option value="1" selected>1×</option></select></label><button id="watch" disabled>Watch (1 of 3)</button><span id="viewCount">0 of 3 viewings completed</span></div>
...
let trial=null,motions=[],current=0,frame=0,camera={},stage='login',playing=false,phaseTime=0,rate=1,last=0,primaryID=null,inspectStart=0,inspected=null,marks=[],drag=null,telem=null,viewings=0;
...
function showStage(s){stage=s;let pre=['ready','question','initial','starting','confirming'].includes(s),canPlay=['ready','question'].includes(s)&&viewings<3;$('preControls').hidden=!pre;$('watch').disabled=!canPlay;$('preSpeed').disabled=!canPlay;$('watch').textContent=viewings>=3?'Three viewings complete':viewings===0?'Watch (1 of 3)':'Watch again ('+(viewings+1)+' of 3)';$('viewCount').textContent=viewings+' of 3 viewings completed';$('primary').hidden=s!=='question';$('inspection').hidden=s!=='inspection';$('skip').hidden=s!=='interrupted'}
async function next(){playing=false;blank();showStage('loading');let v=await api('next');if(!v.trial){status(v.message);return}trial=v.trial;motions=[v.motion];current=0;frame=0;phaseTime=0;rate=1;viewings=0;primaryID=null;camera=clone(trial.viewing_settings.default_camera);inspected=null;marks=[];$('interval').textContent='';$('context').textContent=trial.evaluation_goal+' '+trial.style_context;$('speed').value='1';$('preset').value='three_quarter';$('overlay').checked=false;$('part').value='';$('confidence').value='';document.querySelectorAll('#tags input').forEach(x=>x.checked=false);document.querySelectorAll('.selected').forEach(x=>x.classList.remove('selected'));status('Trial '+trial.display_index+' of '+trial.total+'. Watch up to three times, then answer.');showStage('ready')}
$('begin').onclick=()=>safely(async()=>{await api('login',{rater_id:$('rater').value});$('login').hidden=true;await next()});
$('watch').onclick=()=>safely(async()=>{if(!['ready','question'].includes(stage)||viewings>=3)return;rate=Number($('preSpeed').value);blank();showStage('starting');$('viewportWrap').hidden=false;$('view').hidden=false;$('viewportWrap').scrollIntoView({block:'center'});try{await api('start',{trial_id:trial.trial_id,playback_rate:rate})}catch(e){blank();showStage('interrupted');throw e}if(stage!=='starting')return;showStage('initial');phaseTime=0;frame=0;last=performance.now();playing=true;draw();status('Viewing '+(viewings+1)+' of 3 at '+rate+'×. Answer afterward, or watch again if you need to.')});
for(const b of document.querySelectorAll('#primaryAnswers button'))b.onclick=()=>safely(async()=>{if(stage!=='question')return;showStage('saving');let v;try{v=await api('notice',{trial_id:trial.trial_id,repeated_view_notice:b.dataset.notice})}catch(e){showStage('question');throw e}primaryID=v.observation_id;rate=1;inspectStart=performance.now();telem={format_version:'motionlab.inspection_telemetry.v1',replay_count:[0],speed_change_events:[],playback_wall_time_by_speed_ms:{'0.25':0,'0.5':0,'1':0,'1.5':0,'2':0},camera_change_events:[],zoom_change_count:0,view_change_count:0,overlay_change_count:0};showStage('inspection');$('viewportWrap').hidden=false;$('view').hidden=false;draw();status('Answer saved: '+b.dataset.notice+' after '+viewings+' viewing(s). Inspect if useful, or save and continue.')});
...
async function tick(t){let dt=Math.max(0,(t-last)/1000);last=t;if(playing){if(stage==='initial'&&dt>.3){interrupt()}else{phaseTime+=dt*rate;if(stage==='inspection')telem.playback_wall_time_by_speed_ms[String(rate)]+=dt*1000;if(phaseTime>=trial.duration_s){playing=false;phaseTime=trial.duration_s;if(stage==='initial'){blank();showStage('confirming');try{await api('complete',{trial_id:trial.trial_id});viewings++;showStage('question');status(viewings<3?'You can answer now or watch again ('+(3-viewings)+' remaining).':'Three viewings complete. Please answer based on what you noticed.')}catch(e){showStage('interrupted');$('error').textContent=e.message}}}frame=Math.min(motions[0].positions.length-1,Math.floor(phaseTime*motions[0].fps));if(['initial','inspection'].includes(stage))draw()}}requestAnimationFrame(tick)}
window.__noticeabilityState=()=>({stage,frame,playing,rate,viewings,camera:clone(camera),primaryID,trial});
__RENDERER__
